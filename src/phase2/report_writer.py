...
def write_generation_report(path, *, yaml_path, phase1_output, ddl_text, model, rows_per_table, excel_output, validation):
    ...
    lines.extend(["", "## Lineage Mapping Summary", ""])
    lines.append(f"- Generation engine: {stats.get('generation_engine', 'python')}")
    lines.append(f"- Warehouse generation mode: {stats.get('warehouse_generation_mode', 'lineage-aware')}")
    lines.extend([f"- Raw/load table: {item}" for item in stats.get("lineage_raw_tables", [])] or ["- Raw/load table: None"])
    lines.extend([f"- Staging table: {item}" for item in stats.get("lineage_staging_tables", [])] or ["- Staging table: None"])
    lines.extend([f"- Dimension table: {item}" for item in stats.get("lineage_dimension_tables", [])] or ["- Dimension table: None"])
    lines.extend([f"- Fact table: {item}" for item in stats.get("lineage_fact_tables", [])] or ["- Fact table: None"])
    lines.extend([f"- Entity: {item}" for item in stats.get("lineage_entities", [])] or ["- Entity: None"])
    lines.extend([f"- Raw/Staging: {item}" for item in stats.get("lineage_raw_to_staging", [])] or ["- Raw/Staging: None"])
    lines.extend([f"- Staging/Dimension: {item}" for item in stats.get("lineage_staging_to_dimension", [])] or ["- Staging/Dimension: None"])
    lines.extend([f"- Fact/Dimension: {item}" for item in stats.get("lineage_fact_to_dimension", [])] or ["- Fact/Dimension: None"])
    lines.extend([f"- Derived rows: {item}" for item in stats.get("lineage_derivations", [])] or ["- Derived rows: None"])
    lines.extend([f"- Fact key resolution: {item}" for item in stats.get("lineage_fact_key_resolutions", [])] or ["- Fact key resolution: None"])
...
def write_validation_report(path, pre_validation, post_validation=None):
    ...
    lineage = pre_validation.get("lineage_validation", {})
    lines.extend(["", "## Lineage Validation", ""])
    lines.append(f"Status: **{lineage.get('status', 'not_run')}**")
    lines.extend([f"- Raw/Staging checked: {item}" for item in lineage.get("checked_raw_to_staging", [])] or ["- Raw/Staging checked: None"])
    lines.extend([f"- Staging/Dimension checked: {item}" for item in lineage.get("checked_staging_to_dimension", [])] or ["- Staging/Dimension checked: None"])
    lines.extend([f"- Fact/Dimension checked: {item}" for item in lineage.get("checked_fact_to_dimension", [])] or ["- Fact/Dimension checked: None"])
    lines.extend([f"- {item}" for item in lineage.get("errors", [])] or ["- No lineage errors."])
