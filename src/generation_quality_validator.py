...
def _has_reporting_view(text):
    return bool(re.search(r"\bCREATE\s+(?:MATERIALIZED\s+)?VIEW\b", text, re.IGNORECASE)) or "reporting" in text.lower()
...
def validate_generation_quality(generation_response, model_intent, model_blueprint):
    ...
    checks = {
        ...
        "reporting_views_present": _has_reporting_view(text),
    ...
    if model_type in ANALYTICAL_TYPES:
        ...
        if "reporting" in required_layers and not checks["reporting_views_present"]:
            warnings.append("Reporting layer requested but no reporting view was detected.")
